...
{
    private const STORAGE_DIR = 'projects';
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($files && count($files) > 0) {
            $validated['image'] = $files[0]->store(self::STORAGE_DIR, 'public');
            $gallery = [];
            for ($i = 1; $i < count($files); $i++) {
                $gallery[] = $files[$i]->store(self::STORAGE_DIR, 'public');
            }
    ...
    public function edit(Project $project): View
    {
        ...
        $districts = District::with('city')->orderBy('name')->get();
        $neighborhoods = Neighborhood::with('district')->orderBy('name')->get();
        return view('admin.projects.edit', compact('project', 'cities', 'districts', 'neighborhoods'));
    }
    ...
    public function update(Request $request, Project $project): RedirectResponse
    {
        ...
        if ($request->boolean('remove_main')) {
            ...
            $validated['image'] = $request->file('image')->store(self::STORAGE_DIR, 'public');
        }
        ...
        if ($galleryNew && count($galleryNew) > 0) {
            foreach ($galleryNew as $file) {
                $validated['gallery'][] = $file->store(self::STORAGE_DIR, 'public');
            }
        }
